    ...
    <div class="roles-permissions">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                <div class="w-3/12 px-4 py-3">Case Title</div>
                <div class="w-3/12 px-4 py-3">Case File Date</div>
                <div class="w-2/12 px-4 py-3">First Hearing</div>
            ...
            @foreach ($kesis as $kesi)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $kesi->title }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $kesi->filedate }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $kesi->first_hearing}}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $kesi->next_hearing }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('kesi.show',$kesi->id) }}" class="ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm" title="assign subject">
                        ...
                        <a href="{{ route('kesi.edit',$kesi->id) }}" class="ml-1">
                        ...
                        <a href="{{ route('kesi.destroy', $kesi->id) }}" data-url="{!! URL::route('kesi.destroy', $kesi->id) !!}" class="deletecase ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
...
<script>
    $(function() {
        $( ".deletecase" ).on( "click", function(event) {
